...
void atmosphereEngine::updateAtmosphere(planeStatusData *state)
{
    ...
    if(potential_altitude <= 11000){ //TROPO
        state->atmosphere.t = min(T_SEALEVEL+state->atmosphere.ISADiff,T_SEALEVEL+state->atmosphere.ISADiff + DELTA_T_H*potential_altitude);
        state->atmosphere.p = min(P_SEALEVEL*exp(P_CTE*log(T_SEALEVEL/state->atmosphere.t)),(double)P_SEALEVEL);
